...
# Define the "Pure Brain" channels based on our XGBoost/Separability analysis
PURE_BRAIN_CHANNELS = [
        "f7", "f3", "fz", "f4", "f8",
        "t3", "c3", "cz", "c4", "t4",
        "t5", "p3", "pz", "p4", "t6",
        "o1", "o2"
    ] # Removed fp1, fp2, a1, a2
...
class CacheConfig:
    ...
    # montage
    target_montage_channels: Tuple[str, ...] = tuple(PURE_BRAIN_CHANNELS)
...
def assign_raw_label_by_overlap(
    ...
    for item in intervals:
        ...
        if overlap > 0:
            # If ANY seizure activity exceeds the threshold (e.g., 1 second), 
            # label the whole window as seizure immediately.
            if "sz" in lab or "seiz" in lab:
                if (overlap / window_duration) >= seizure_priority_threshold:
                    return lab 
