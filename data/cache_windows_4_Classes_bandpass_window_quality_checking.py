...
def is_bad_window(
    ...
    # per-channel std across time
    ch_std = xw.std(axis=1)

    # channel is flat if std is extremely small
    flat_mask = ch_std < flat_std_thresh

    # channel is zero-padded if all samples are near zero
    zero_mask = np.all(np.abs(xw) < 1e-8, axis=1)

    flat_ratio = flat_mask.mean()
...
if __name__ == "__main__":
    manifest = build_cache_from_json(
        json_path=r"C:\Users\basel\Desktop\testing eeg codes\eeg_seizure_only.json",
        out_dir=r"C:\Users\basel\Desktop\testing eeg codes\cache_windows",
        fs=250,
        window_sec=10.0,
        stride_sec=5.0,
        c_max=41,
